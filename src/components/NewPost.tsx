...
const NewPost = ({ musicQuery, setMusicQuery }: NewPostProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (musicQuery.lyric == null || musicQuery.lyricComment == null || musicQuery.videoId == null) {
        alert("가사, 번안가사, 유튜브 주소는 필수 입력 사항입니다.");
        return;
      }
      ...
      await console.log("서버 응답 데이터:", response.data);
      navigate("/");
    } catch (error) {
      // 요청 실패 시의 처리
